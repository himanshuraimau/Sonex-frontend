...
export function SoundEffectsGenerator({ credits }: { credits: number }) {
  ...
  const handleGenerateSoundEffect = async () => {
    ...
    try {
      setLoading(true);
      const { audioId, shouldShowThrottleAlert } = await generateSoundEffect(
        textContent
      );
    ...
    <>
      ...
      <div className="relative flex h-full w-full flex-col items-center">
        ...
        <div className="relative z-10 flex h-full w-full flex-col items-center gap-10 md:pt-20">
          <div
            className={`h-fit w-full max-w-2xl rounded-xl border bg-white p-4 shadow-xl transition-colors duration-200 ${
